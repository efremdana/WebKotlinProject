...
import ru.ac.uniyar.models.RegistrationPageVM
...
class RegistrationViewHandler(
    ...
    override fun invoke(request: Request): Response {
        ...
        val webForm = WebForm()
        val model = RegistrationPageVM(webForm, listOf(), user, permissions)
...
class RegistrationNewUser(
    ...
    override fun invoke(request: Request): Response {
        val permissions = access(request)
        val user = key(request)
        val webForm = registrationForm(request)
        ...
        if (!validateForm.validatePassword(password, passwordCheck) &&
            ...
            val model = RegistrationPageVM(webForm, validateForm.getErrorsList(), user, permissions)
